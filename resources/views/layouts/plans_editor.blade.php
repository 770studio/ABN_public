<script>
    $(document).ready(function () {
        ...
        function fetchPlans() {
            $.ajax({
                url: "{{config('app.url')}}/planseditor/plansFetch",
                dataType: "json",
        ...
        function uploadUsers() {
            $.ajax({
                url: "{{config('app.url')}}/planseditor/plansUploadUsers",
                method: "POST",
                data: {_token: _token},
                dataType: "json",
                success: function (data) {
        ...
        $(document).on('click', '#add', function () {
            ...
            if (employee_id != '' && plans != '' && year != '') {
                $.ajax({
                    url: "{{config('app.url')}}/planseditor/plansAdd",
                    method: "POST",
        ...
        $(document).on('change', '.update', function () {
            ...
            if (confirm("Вы уверены, что хотите обновить текущий план?")) {
                $.ajax({
                    url: "{{config('app.url')}}/planseditor/plansUpdate",
                    method: "POST",
        ...
        $(document).on('click', '.delete', function () {
            ...
            if (confirm("Вы уверены, что хотите удалить текущий план?")) {
                $.ajax({
                    url: "{{config('app.url')}}/planseditor/plansDelete",
                    method: "POST",
